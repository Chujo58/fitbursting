...
class LSFitter:
    ...
    def compute_hessian(self, data: float, parameter_list: list) -> float:
        ...
        for current_par in self.fit_parameters:
            if np.any([current_par == x for x in self.global_parameters]):
                ...
                par_labels_output += [
                    f"{current_par}{idx + 1}"
                    for idx in range(self.model.num_components)
                ]
        ...
        for current_par_idx_1, current_par_1 in zip(range(num_par), par_labels):
            ...
            for current_par_idx_2, current_par_2 in zip(
                range(current_par_idx_1, num_par), par_labels[current_par_idx_1:]
            ):
                ...
                try:
                    current_mixed_deriv = getattr(
                        deriv, f"deriv2_model_wrt_{current_par_1}_{current_par_2}"
                    )

                except AttributeError:
                    current_mixed_deriv = getattr(
                        deriv, f"deriv2_model_wrt_{current_par_2}_{current_par_1}"
                    )
                ...
                if (
                    (current_par_1 not in self.global_parameters)
                    and (current_par_2 not in self.global_parameters)
                    and (
                        par_labels_idxs[current_par_idx_1]
                        != par_labels_idxs[current_par_idx_2]
                    )
                ):
                    pass
                ...
                current_hes = 2 * (
                    current_deriv_1 * current_deriv_2 - residual * current_deriv_mixed
                )
                hessian[current_par_idx_1, current_par_idx_2] = np.sum(
                    current_hes * self.weights[:, None] ** 2
                )
                hessian[current_par_idx_2, current_par_idx_1] = hessian[
                    current_par_idx_1, current_par_idx_2
                ]
    ...
    def compute_jacobian(self, parameter_list: list) -> float:
        ...
        for current_parameter in self.fit_parameters:
            ...
            for current_component in range(num_derivs):
                ...
                jacobian[:, current_parameter_idx] = (
                    current_jac * self.weights[:, None]
                ).flat[:]
    ...
    def fit(self, exact_jacobian: bool = True) -> None:
        ...
        try:
            results = least_squares(
                self.compute_residuals, parameter_list, jac=jac, workers=1
            )
    ...
    def _compute_fit_statistics(
        self, spectrum_observed: float, fit_result: object
    ) -> None:
        """
        ...
        self.fit_statistics["num_observations"] = (
            num_freq_good * int(num_time) - num_fit_parameters
        )
        ...
        try:
            ...
            if np.any(np.isnan(uncertainties)):
                print(
                    "WARNING: one or more NaNs in exact uncertainties, replacing with approximates..."
                )
                uncertainties = [
                    float(x) for x in np.sqrt(np.diag(covariance_approx)).tolist()
                ]
            ...
            self.fit_statistics["bestfit_uncertainties"] = (
                self.load_fit_parameters_list(uncertainties)
            )
            self.fit_statistics["bestfit_covariance"] = (
                None  # return the full matrix at some point?
            )
    ...
    def _set_weights(self) -> None:
        ...
        if self.weighted_fit:
            self.weights[self.good_freq] = 1.0 / std[self.good_freq]

        else:
            self.weights[self.good_freq] = 1.0

        # zero-weight bad channels.
        self.weights[bad_freq] = 0.0
